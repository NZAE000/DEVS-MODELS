...
void JobManager_t::deployJob(std::vector<shared_ptr<dynamic::modeling::model>>& nodes) noexcept
{
    ...
    {   
        ...
        for (auto i=0; i<replica; i++)
        {
            ...
            if (++node_iter == end(subclase_nodes)) node_iter = subclase_nodes.begin();   // Next node (if is end, then go back to begin node)
            node = *node_iter.base();
        }
    }
...
JobManager_t::getOperLocation_balanced(operId_t const& oper_id) const noexcept
{
    ...
    for (auto const loc : locations)
    {
        nTuple = resourceMan_.nTupleQueue(loc); // Know n tuples from slot´s queue of node.
        if (nTuple < less) {
            less = nTuple;
